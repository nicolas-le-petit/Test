...
none_tutor = ["No Tutor is available now!"];
...
function update_form(){
  ...
  for (var i=0; i<items.length; i++){
    if (items[i].getTitle() == "Please Select Your Tutor"){
      ...
      if (tutor == false) 
        {pList.setChoiceValues(none_tutor).setRequired(true);
          return}
      else
...
function booking(){  
  ...
  for (var i = 1; i < data.length ; i++) {
    ...
    if (data[i][6] == tutor_booked_infor & data[i][5] == ""){
      ...
      var tutor_phone = find_feedback(link_Data_base,sheet_email,1,tutor_booked,4);//Tutor phone
